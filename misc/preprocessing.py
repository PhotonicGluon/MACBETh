...
def get_unigrams_from_report(report: dict) -> List[str]:
    ...
    unigrams = list(set(filter(None, behaviours_data.split())))
...
def get_top_unigrams(unigram_counts: Dict[str, int], num=1000, seed=-1) -> List[str]:
    ...
    :param num: number of unigrams to find, defaults to 1000
    :param seed: the seed for randomising the output list. If `-1` then no randomisation will be
...
def unigram_list_to_coordinates(
    unigram_list: List[str], top_unigrams: List[str]
) -> List[int]:
    """
...
def reports_to_coordinates(
    data: List[dict], num_unigrams=1000, seed=-1
) -> Tuple[np.ndarray, List[str]]:
    """
    Converts all the reports into coordinates.

    :param data: data array containing the reports
    :param num_unigrams: number of unigrams to include in the coordinates, defaults to 1000
    :param seed: the seed for randomising the output list. If `-1` then no randomisation will be
    performed. Defaults to -1
    :return: an array of coordinates. The size of the array will be `(len(data), num_unigrams)`
    :return: the list of the top `num_unigrams` unigrams, in the same order as the array of
    coordinates
    """
    ...
    coordinates = np.array(
        [
            unigram_list_to_coordinates(unigram_list, top_unigrams)
